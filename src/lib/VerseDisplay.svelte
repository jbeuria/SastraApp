<script>
    ...
    let {
        ...
        currentSlideIndex = $bindable(0) // Bind currentSlideIndex for presentation mode
    } = $props();
    ...
    function handleContentClick(event) {
        ...
        if (event.target.tagName === 'MARK') {
            ...
            if (clickedHighlight) {
                ...
                currentSelection = null; // Clear any active text selection
        ...
        setTimeout(() => {
            ...
            if (selection && !selection.isCollapsed && selection.toString().trim().length > 0) {
                ...
                range = expandRangeToWordBoundaries(range); // Expand to full words
                selection.removeAllRanges(); // Clear original selection
                selection.addRange(range); // Add expanded selection
                currentSelection = range;
                highlightToDelete = null; // Clear any highlight to delete
                const rect = currentSelection.getBoundingClientRect();
                popupCoords = { x: rect.left + rect.width / 2, y: rect.top };
                showKrishnaPopup = true;
            } else {
                showKrishnaPopup = false; // Hide popup if no selection
            }
        }, 50);
    }
    ...
    async function createHighlight() {
        ...
        const id = await db.highlights.add(newHighlight); // Add to IndexedDB
        highlights = [...highlights, { ...newHighlight, id }]; // Update reactive state
    ...
    async function deleteHighlight() {
        ...
        await db.highlights.delete(highlightToDelete.id); // Delete from IndexedDB
        if (highlightToDelete.id) {
            try {
                await deleteHighlightFromSupabase(highlightToDelete); // Sync deletion to Supabase
            } catch (e) {
                // Log but don't prevent local deletion on sync failure
            }
        }
        highlights = highlights.filter((h) => h.id !== highlightToDelete.id); // Update reactive state
        showKrishnaPopup = false;
        highlightToDelete = null;
    }

    // Applies highlights to a given text content
    ...
    // Toggles visibility settings for presentation content, ensuring at least one is always selected
    function handleVcToggle(key) {
        if (settings.vc[key] === true) {
            const checkedCount = Object.values(settings.vc).filter(Boolean).length;
            if (checkedCount <= 1) return; // Prevent unchecking the last visible item
        }
        settings.vc[key] = !settings.vc[key];
    }

    // Chunks long text into smaller paragraphs for presentation mode
    function chunkText(text, maxLength) {
        ...
        while (remainingText.length > 0) {
            ...
            if (splitIndex > maxLength / 2) { // Ensure chunk is not too small
                chunk = remainingText.substring(0, splitIndex + 1);
            } else {
                splitIndex = chunk.lastIndexOf(' '); // Fallback to word boundary
    ...
    // Recalculates slides for presentation mode whenever settings or data change
    $effect(() => {
        const mode = settings.mode;
        const data = selectedData;
        const vc = settings.vc;
        // Only generate slides if in PPT mode, data is available, and window size is known
        if (mode !== 'ppt' || !data || data.length === 0 || windowSize.width === 0) {
            slides = [];
            return;
        }
        const screenArea = windowSize.width * windowSize.height;
        // Adjust max length dynamically based on screen size for better readability
        ...
        for (const verse of data) {
            ...
            if (vc.showPurport && verse.purport) {
                // Split purport by newlines, then chunk each paragraph
        ...
        slides = newSlides;
        currentSlideIndex = 0; // Reset to first slide when slides are regenerated
    });

    // Fetches book content and user data (highlights, notes, bookmarks) on tocURL change
    $effect(async () => {
        stopReading(); // Stop any ongoing speech when content changes
        if (!tocURL) { isLoading = false; selectedData = []; return; }
        isLoading = true; errorMessage = ''; selectedData = [];
        try {
            let dataToProcess;
            const localContent = await db.content.get(tocURL); // Try fetching from IndexedDB
            if (localContent && !isCollection) {
                dataToProcess = [localContent];
            } else {
                // Fetch from Supabase if not in IndexedDB or it's a collection
                ...
                if (onlineData.length > 0) await db.content.bulkPut(onlineData); // Store in IndexedDB
            }
            selectedData = dataToProcess;
            currentTitle = dataToProcess[0]?.verse_url || tocURL; // Set title for notes/bookmarks
            highlights = await db.highlights.where('toc_url', tocURL); // Load highlights for current page using your custom 'where'
            await checkBookmarkStatus();
            const existingNote = await db.notes.get(tocURL); // Load existing note
    ...
    // Saves the current note to IndexedDB and syncs to Supabase
    ...
    // Toggles bookmark status for the current page
    ...
    // Derived state for the current slide in PPT mode
    const currentSlide = $derived(slides[currentSlideIndex]);
    // Derived state for presentation progress
    ...
    onMount(() => {
        // Update window size on resize for responsive slide generation
    ...
    onDestroy(() => {
        stopReading(); // Stop speech synthesis when component is destroyed
    });
</script>
